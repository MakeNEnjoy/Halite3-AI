...
while True:
		...
		for x in range(game_map.width):
			for y in range(game_map.height):
				if(game_map[Position(x, y)].halite_amount / 4 ** (game_map.calculate_distance(ship.position, Position(x, y))) >
                                        game_map[Position(*high_halite)].halite_amount / 4 ** (
                                    game_map.calculate_distance(ship.position, Position(*high_halite)))
	...
	if game.turn_number <= constants.MAX_TURNS * 0.4 and me.halite_amount >= constants.SHIP_COST and not game_map[me.shipyard].is_occupied:
